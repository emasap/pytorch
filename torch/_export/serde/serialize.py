...
def serialize_operator(target) -> str:
    if isinstance(target, str):
        return target
    elif target.__module__.startswith("torch._ops"):  # From torch.fx.node._get_qualified_name
        ...
        for serialized_node in serialized_graph.nodes:
            try:
                ...
                if target.__module__ == "_operator": # TODO(zhxchen17) Follow up on this.
                    ...
                    assert len(serialized_node.outputs) == 1 and serialized_node.outputs[0].as_tensor is not None, "Only single tensor output is supported for higher order operators."
